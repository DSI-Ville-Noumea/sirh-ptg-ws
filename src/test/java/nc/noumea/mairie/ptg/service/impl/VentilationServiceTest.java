...
public class VentilationServiceTest {
	...
	public void processVentilation_VentilationDateExisting_useExistingOne() {
		...
		Mockito.doReturn(ptgVentiles).when(service).processHSupAndAbsVentilationForWeekAndAgent(lastUnPaidVentilDate, 9005432, carr, datesLundi.get(0), lastPaidDate);
		...
		Mockito.doReturn(ptgVentiles).when(service).processPrimesVentilationForMonthAndAgent(lastUnPaidVentilDate, 9005432, datesDebutMois.get(0), lastPaidDate);
		...
		Mockito.verify(service, Mockito.times(1)).processHSupAndAbsVentilationForWeekAndAgent(lastUnPaidVentilDate, 9005432, carr, datesLundi.get(0), lastPaidDate);
		...
		Mockito.verify(service, Mockito.times(1)).processPrimesVentilationForMonthAndAgent(lastUnPaidVentilDate, 9005432, datesDebutMois.get(0), lastPaidDate);
	...
	public void processVentilationForAgent_NoPointageType_DoAll() {
		...
		IVentilationRepository vRepo = Mockito.mock(IVentilationRepository.class);
		Mockito.when(vRepo.getDistinctDatesOfPointages(9005432, fromVentilDate.getDateVentilation(), ventilationDate)).thenReturn(pointagesDates);
		...
		List<Pointage> ptgVentiles = new ArrayList<Pointage>();
		Mockito.doReturn(ptgVentiles).when(service).processHSupAndAbsVentilationForWeekAndAgent(toVentilDate, 9005432, carr, datesLundi.get(0), fromVentilDate.getDateVentilation());

		Mockito.doNothing().when(service).removePreviousCalculatedPointages(9005432, datesLundi.get(0));
		Mockito.doNothing().when(service).calculatePointages(9005432, datesLundi.get(0), fromVentilDate.getDateVentilation(), toVentilDate.getDateVentilation());
		Mockito.doReturn(ptgVentiles).when(service).processPrimesVentilationForMonthAndAgent(toVentilDate, 9005432, datesDebutMois.get(0), fromVentilDate.getDateVentilation());
		...
		Mockito.verify(service, Mockito.times(2)).getDistinctDateLundiFromListOfDates(pointagesDates);
		Mockito.verify(service, Mockito.times(1)).getDistinctDateDebutMoisFromListOfDates(pointagesDates);
		Mockito.verify(service, Mockito.times(1)).processHSupAndAbsVentilationForWeekAndAgent(toVentilDate, 9005432, carr, datesLundi.get(0), fromVentilDate.getDateVentilation());
		Mockito.verify(service, Mockito.times(1)).removePreviousCalculatedPointages(9005432, datesLundi.get(0));
