...
public class AlfrescoCMISService implements IAlfrescoCMISService {
	...
	@Override
	public void uploadDocument(Integer idAgentOperateur, byte[] bFile, String titreFile, String descriptionFile, TypeEtatPayeurPointageEnum typeEtatPayeur) {
		...
		if(null != doc.getProperty("cmis:secondaryObjectTypeIds")) {
			...
			if (!aspects.contains("P:mairie:customAgentAspect")) {
				aspects.add("P:mairie:customAgentAspect");
		...
		HashMap<String, Object> props = new HashMap<String, Object>();
		props.put("mairie:idAgentAspect", idAgentOperateur);
